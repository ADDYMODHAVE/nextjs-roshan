import React from "react";
import { OrderBookingFlash } from "./OrderBookingFlash";
import Link from "next/link";

export const PriceCard = () => {
  return (
    <>
      <OrderBookingFlash />
      <div className="d-flex flex-column flex-md-row align-items-center justify-content-md-center m-2">
        <div
          className="card m-2"
          style={{ width: "18rem", background: "rgb(212,175,55)" }}
        >
          {/* <img
          src="data:image/jpeg;base64,"
          className="card-img-top"
          alt="..."
        /> */}
          <div className="card-body d-flex flex-column justify-content-center">
            <h5 className="card-title overflow-visible text-center fw-bolder">
              Advance
            </h5>
            <div>
              <h6 className="card-title overflow-visible text-center fw-bolder">
                Features
              </h6>
              <ul className="list-group list-group-flush border border-black rounded m-2 fw-bold p-2">
                <li className="ms2">Traditional Photos</li>
                <li className="ms2">Traditional Video</li>
                <li className="ms2">Cinematic Video</li>
                <li className="ms2">Candid Photo</li>
                <li className="ms2">Drone</li>
                <li className="ms2">Screen</li>
                <li className="ms2">35 pages Traditional Album</li>
                <li className="ms2">10-15 pages Candid Album</li>
                <li className="ms2">Some Gifts From Our Team</li>
              </ul>
              <h6 className="card-title overflow-visible text-center fw-bolder">
                70K
              </h6>
            </div>

            <Link href="/book" className="btn btn-success">
              BOOK ORDER
            </Link>
          </div>
        </div>
        <div className="card m-2" style={{ width: "18rem" }}>
          {/* <img
          src="data:image/jpeg;base64,"
          className="card-img-top"
          alt="..."
        /> */}
          <div
            className="card-body d-flex flex-column justify-content-center"
            style={{ background: "rgb(217,233,127)" }}
          >
            <h5 className="card-title overflow-visible text-center fw-bolder">
              Medium
            </h5>
            <div>
              <h6 className="card-title overflow-visible text-center fw-bolder">
                Features
              </h6>
              <ul className="list-group list-group-flush border border-black rounded m-2 fw-bold p-2">
                <li className="ms2">Traditional Photos</li>
                <li className="ms2">Traditional Video</li>
                <li className="ms2">Candid Photos</li>
                <li className="ms2">Drone</li>
                <li className="ms2">30 pages Album</li>
                <li className="ms2">Some Gifts</li>
              </ul>
              <h6 className="card-title overflow-visible text-center fw-bolder">
                40k
              </h6>
            </div>

            <Link href="/book" className="btn btn-success">
              BOOK ORDER
            </Link>
          </div>
        </div>
        <div className="card m-2" style={{ width: "18rem" }}>
          {/* <img
          src="data:image/jpeg;base64,"
          className="card-img-top"
          alt="..."
        /> */}
          <div
            className="card-body d-flex flex-column justify-content-center"
            style={{ background: "rgb(233,237,211)" }}
          >
            <h5 className="card-title overflow-visible text-center fw-bolder">
              Basic
            </h5>
            <div>
              <h6 className="card-title overflow-visible text-center fw-bolder">
                Features
              </h6>
              <ul className="list-group list-group-flush border border-black rounded m-2 fw-bold p-2">
                <li className="ms2">Traditional Photos</li>
                <li className="ms2">Traditional Video</li>
                <li className="ms2">25 pages Album</li>
                <li className="ms2">Some Basic Gift</li>
              </ul>
              <h6 className="card-title overflow-visible text-center fw-bolder">
                25k
              </h6>
            </div>

            <Link href="/book" className="btn btn-success">
              BOOK ORDER
            </Link>
          </div>
        </div>
      </div>
    </>
  );
};
